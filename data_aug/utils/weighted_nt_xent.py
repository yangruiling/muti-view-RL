    ...
    def forward(self, x1, x2, mols):
        ...
        for i in range(len(mols)):
            for j in range(i+1, len(mols)):
                fp_sim = DataStructs.TanimotoSimilarity(fps[i], fps[j]) #两两计算当前batch内的分子相似度
                fp_score[i,j-1] = fp_sim # 记录相似度得分
                fp_score[j,i] = fp_sim
        ...
        similarity_matrix = self.similarity_function(representations, representations) # 计算两个增强图的相似度矩阵
