...
class GINet_3D(nn.Module):
    def __init__(self, num_layer=5, emb_dim=300, feat_dim=256, dropout=0, pool='mean'):
        ...
        self.x_embedding1 = nn.Embedding(num_atomsymbol, emb_dim)
        self.x_embedding2 = nn.Embedding(num_degree, emb_dim)
        self.x_embedding3 = nn.Embedding(num_HybridizationType, emb_dim)
        self.x_embedding4 = nn.Embedding(num_implicitvalence, emb_dim)
        self.x_embedding5 = nn.Embedding(num_formal_charge, emb_dim)
        self.x_embedding6 = nn.Embedding(num_atomic, emb_dim)
        self.x_embedding7 = nn.Embedding(num_Hs, emb_dim)
        self.x_embedding8 = nn.Embedding(num_inringsize, emb_dim)
        self.x_embedding9 = nn.Embedding(num_chirality, emb_dim)
        self.x_embedding10 = nn.Embedding(num_ishydorogendonor, emb_dim)
        self.x_embedding11 = nn.Embedding(num_ishydorogenaccector, emb_dim)
        self.x_embedding12 = nn.Embedding(num_isacidic, emb_dim)
        self.x_embedding13 = nn.Embedding(num_isbasic, emb_dim)
    ...
    def forward(self, data):
        ...
        h_global = self.pool(h, data.batch) #全局池化
        h_global = self.feat_lin(h_global) # 特征拉平
        out_global = self.out_lin(h_global) #输出
